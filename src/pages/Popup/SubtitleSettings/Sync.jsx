...
const Sync = ({ popup ,index}) => {
  ...
  function handleSync(synchronization) {
    if (popup) {
      ...
      const syncNow = new CustomEvent('syncNow'+index, {
  ...
  if (!popup && !listening) {
    ...
    chrome.runtime.onMessage.addListener((msg) => {
      if (msg.syncValue && msg.index==index) {
      ...
      <Container>
        ...
        <Box my={4}>
          <Grid container justify="center">
            <Button
              onClick={() =>
                handleSync({ syncValue: syncValue, syncLater: direction, index:index })
              }
              variant="contained"
              color="primary"
              endIcon={<SyncIcon />}
